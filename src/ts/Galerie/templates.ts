...
import { Foto, loadImage } from "../gemeinsam";
...
function AbbildungBauen(Foto: Foto) {
    ...
    return html`
        <figure ?Liebling=${Foto.IstLiebling}>
            ...
            <a href="/foto/${Foto.ID}" class="Bild-Link spinner">
...
function GalerieGridBauen(Fotos: Foto[]) {
    return html`${Fotos.map(AbbildungBauen)}`;
}
...
function Galerie() {
    return html`
        ...
        <header class="Galerie__Kopf Flexible Flexible--a-center">
            <h2 class="Titel--sekundär">Galeria</h2>
        ...
        <div class="Galerie__Bilden">${GalerieGridBauen(Bilden)}</div>
    </section>`;
}
